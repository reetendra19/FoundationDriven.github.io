	...
	b.fn.fullpage = function (c) {
		...
		function N() {
			...
			if (!c.autoScrolling && !f.hasClass("active")) {
				C = !0;
				var D = b(".fp-section.active").index(".fp-section") + 1,
					g = E(f),
					h = f.data("anchor");
				f.addClass("active").siblings().removeClass("active");
				k || (b.isFunction(c.onLeave) && c.onLeave.call(this, D, f.index(".fp-section") + 1, g), b.isFunction(c.afterLoad) && c.afterLoad.call(this, h, f.index(".fp-section") + 1));
				F(h, 0);
		...
		function x(a, d) {
			...
			else c = "top", f = b.fn.fullpage.moveSectionUp; if (0 < d.length)
		...
		function la(a) {
			...
			if (!R(a.target)) {
				...
				var e = Q(a);
				k || t || (d = S(d), r = d.y, y = d.x, a.find(".fp-slides").length && Math.abs(z - y) > Math.abs(s - r) ? Math.abs(z - y) >
					b(window).width() / 100 * c.touchSensitivity && (z > y ? b.fn.fullpage.moveSlideRight() : b.fn.fullpage.moveSlideLeft()) : c.autoScrolling && !c.scrollBar && Math.abs(s - r) > b(window).height() / 100 * c.touchSensitivity && (s > r ? x("down", e) : r > s && x("up", e)))
			}
		}
		...
		function n(a, d, e) {
			...
			if ("undefined" !== typeof f && (d = {
				...
				if (d.activeSlide.length) var D = d.activeSlide.data("anchor"),
					g = d.activeSlide.index();
				c.autoScrolling && c.continuousVertical && "undefined" !== typeof d.isMovementUp && (!d.isMovementUp && "up" == d.yMovement || d.isMovementUp && "down" == d.yMovement) && (d.isMovementUp ? b(".fp-section.active").before(d.activeSection.nextAll(".fp-section")) : b(".fp-section.active").after(d.activeSection.prevAll(".fp-section").get().reverse()), l(b(".fp-section.active").position().top), U(), d.wrapAroundElements = d.activeSection, d.dest = d.element.position(), d.dtop = d.dest.top, d.yMovement = E(d.element));
				a.addClass("active").siblings().removeClass("active");
				k = !0;
				V(g, D, d.anchorLink, d.sectionIndex);
		...
		function pa(a) {
			a.wrapAroundElements && a.wrapAroundElements.length && (a.isMovementUp ? b(".fp-section:first").before(a.wrapAroundElements) : b(".fp-section:last").after(a.wrapAroundElements), l(b(".fp-section.active").position().top), U())
		}
			...
			if (!C) {
				...
				if (b.length) {
					var c = "undefined" === typeof q,
						f = "undefined" === typeof q && "undefined" === typeof a && !t;
					(b && b !== q && !c || f || !t && H != a) && I(b, a)
				}
			}
		}
		...
		function u(a, d) {
			...
			if (c.onSlideLeave) {
				...
				q || "none" === r || b.isFunction(c.onSlideLeave) && c.onSlideLeave.call(this, m, k + 1, p, r)
			}
			...
			var s = function () {
				q || b.isFunction(c.afterSlideLoad) && c.afterSlideLoad.call(this, m, k + 1, l, g);
				t = !1
			};
			c.css3 ?
				(e = "translate3d(-" + e.left + "px, 0px, 0px)", $(a.find(".fp-slidesContainer"), 0 < c.scrollingSpeed).css(aa(e)), setTimeout(function () {
				s()
		...
		function ga(a) {
			...
			if (c.paddingTop || c.paddingBottom) b = a, b.hasClass("fp-section") ||
				(b = a.closest(".fp-section")), a = parseInt(b.css("padding-top")) + parseInt(b.css("padding-bottom")), b = h - a;
			return b
		}
		...
		function I(a, c) {
			...
			a === q || e.hasClass("active") ? ha(e, c) : n(e, function () {
				ha(e, c)
			})
		}
		...
		function V(a, b, e, f) {
			var g = "";
			c.anchors.length ? (a ? ("undefined" !== typeof e && (g = e), "undefined" === typeof b && (b = a), H = b, location.hash = g + "/" + b) : ("undefined" !== typeof a && (H = b), location.hash = e), A(location.hash)) : "undefined" !== typeof a ? A(f + "-" + a) : A(String(f))
		}
		...
		(function () {
			c.continuousVertical &&
				(c.loopTop || c.loopBottom) && (c.continuousVertical = !1, B("warn", "Option `loopTop/loopBottom` is mutually exclusive with `continuousVertical`; `continuousVertical` disabled"));
			c.continuousVertical && c.scrollBar && (c.continuousVertical = !1, B("warn", "Option `scrollBar` is mutually exclusive with `continuousVertical`; `continuousVertical` disabled"));
			b.each(c.anchors, function (a, c) {
				(b("#" + c).length || b('[name="' + c + '"]').length) && B("error", "data-anchor tags can not have the same value as any `id` element on the site (or `name` element for IE).")
			})
		})();
		b.extend(b.easing, {
		...
		b.fn.fullpage.setAutoScrolling = function (a) {
			...
			c.autoScrolling && !c.scrollBar ? (b("html, body").css({
				...
				"touch-action": "none"
			}), a.length && l(a.position().top)) : (b("html, body").css({
				overflow: "visible",
				height: "initial"
			}), g.css({
				"-ms-touch-action": "",
				"touch-action": ""
			}), l(0), b("html, body").scrollTop(a.position().top))
		};
		...
		b.fn.fullpage.setAllowScrolling = function (a) {
			if (a) {
				if (b.fn.fullpage.setMouseWheelScrolling(!0), J || ja) MSPointer =
					ia(), b(document).off("touchstart " + MSPointer.down).on("touchstart " + MSPointer.down, ma), b(document).off("touchmove " + MSPointer.move).on("touchmove " + MSPointer.move, la)
		...
		b.fn.fullpage.moveSectionUp = function () {
			...
			a.length || !c.loopTop &&
				!c.continuousVertical || (a = b(".fp-section").last());
			a.length && n(a, null, !0)
		};
		...
		b.fn.fullpage.moveTo = function (a, c) {
			...
			"undefined" !== typeof c ? I(a, c) : 0 < e.length && n(e)
		};
		...
		var t = !1,
			...
			k = !1,
			v = !1,
			q, H, m, Y = "fullpage-wrapper";
		b.fn.fullpage.setAllowScrolling(!0);
		c.css3 && (c.css3 = sa());
		b(this).length ? (g.css({
			height: "100%",
			position: "relative"
		}), g.addClass(Y)) : B("error", "Error! Fullpage.js needs to be initialized with a selector. For example: $('#myContainer').fullpage();");
		...
		b(".fp-section").each(function (a) {
			...
			if (1 < f) {
				...
				c.slidesNavigation && ra(b(this), f);
				e.each(function (a) {
					b(this).css("width", g + "%");
			...
			c.navigation && (m.css("margin-top", "-" + m.height() / 2 + "px"), m.find("li").eq(b(".fp-section.active").index(".fp-section")).find("a").addClass("active"));
			...
			if (a.length) {
				...
				!c.animateAnchor && d.length && (c.autoScrolling ? l(d.position().top) : (l(0), A(a), b("html, body").scrollTop(d.position().top)), F(a, null), b.isFunction(c.afterLoad) && c.afterLoad.call(this,
					a, d.index(".fp-section") + 1), d.addClass("active").siblings().removeClass("active"))
			}
